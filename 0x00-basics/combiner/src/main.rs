...
fn main()->Result<(), ImageDataErrors>{
    let args = Args::new();
    let (image_1, image_1_format) = get_image_from_path(args.image_1)?;
    let (image_2, image_2_format) = get_image_from_path(args.image_2)?;
    
    if image_1_format != image_2_format{
        return Err(ImageDataErrors::DifferentImageFormarts)
    }
...
fn get_smallest_dimensions(dim_1: (u32, u32), dim_2: (u32, u32))->(u32, u32){
    ...
    return if pix_1 > pix_2 {dim_2} else {dim_1}
}
